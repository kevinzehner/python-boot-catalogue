...
def get_models(db_path, manufacturer):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT Model FROM wheelbearing_LSODS WHERE Manuf = ?", (manufacturer,)
    )
...
def get_engine_sizes(db_path, manufacturer, model):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT EngineSize FROM wheelbearing_LSODS WHERE Manuf = ? AND Model = ?",
        (manufacturer, model),
    )
...
def get_mark_series(db_path, manufacturer, model, engine_size):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT mark_series FROM wheelbearing_LSODS WHERE Manuf = ? AND Model = ? AND EngineSize = ?",
...
def get_drive_types(db_path, manufacturer, model, engine_size, mark_series):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT TRWDansDRWDive FROM wheelbearing_LSODS WHERE Manuf = ? AND Model = ? AND EngineSize = ? AND mark_series = ?",
...
def get_positions(db_path, manufacturer, model, engine_size, mark_series, drive_type):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT MPos FROM wheelbearing_LSODS WHERE Manuf = ? AND Model = ? AND EngineSize = ? AND mark_series = ? AND TRWDansDRWDive = ?",
...
def get_parts(
    ...
    cursor = conn.cursor()
    query = """
        SELECT DISTINCT LSODS_PartNumber, Bearing_1_Size, jpeg FROM wheelbearing_LSODS
        WHERE Manuf = ? AND Model = ? AND EngineSize = ? AND mark_series = ? AND TRWDansDRWDive = ? AND MPos = ?
    """
    cursor.execute(
        query, (manufacturer, model, engine_size, mark_series, drive_type, position)
    )
